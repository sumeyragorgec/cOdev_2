...
void ekle(struct oyuncu oyuncular[20], char *karakter, char satir[100], FILE *paslar, char *okunanDeger, char neyeGore[], bool kaydiVarMi, int kacinciOyuncu, int oyuncununIndeksi, int ozellikSayaci) {
    ...
    okunanDeger = strtok(satir, neyeGore); // isim //satir icini kelime kelime okur// neyeGore  parametresi ile gösterilen karakterleri göz ardý ederek, kalan karakterleri diziler halinde ayýrýr
    ...
    if (!kaydiVarMi) {//false ise 
      ...
      while(okunanDeger != NULL) {
        ...
        okunanDeger = strtok(NULL, neyeGore); // bir sonraki kelimeye gelir//ikincisi, belirtilecek dizenin baþlangýcýný ve sonunu tanýmlayacak anahtar kelimeyi veya karakteri tutan sýnýrlayýcýdýr.
